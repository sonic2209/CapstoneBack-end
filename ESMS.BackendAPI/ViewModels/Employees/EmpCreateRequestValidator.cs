    ...
    public class EmpCreateRequestValidator : AbstractValidator<EmpCreateRequest>
    {
        public EmpCreateRequestValidator()
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("Name cannot be empty")
                .MaximumLength(50).WithMessage("Name can not exceed 50 characters").Matches(@"^(?:[^\W\d_]| )+$");//Matches(@"/^[a-zA-Z\s]*$/");

            RuleFor(x => x.Email).NotEmpty().WithMessage("Email cannot be empty").MaximumLength(50).WithMessage("Email can not exceed 50 characters")
